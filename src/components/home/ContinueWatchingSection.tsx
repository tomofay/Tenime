  ...
  return (
    <Section title="Lanjutkan Nonton" variant="horizontal">
      {entries.slice(0, 12).map((entry) => (
        ...
        >
          <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-surface ring-1 ring-black/5 group-hover:ring-accent/40 transition-all">
            {entry.posterUrl ? (
              <Image
                src={entry.posterUrl}
                alt={entry.title}
          ...
          <p className="mt-1.5 text-xs sm:text-sm font-semibold text-foreground truncate group-hover:text-accent-hover transition-colors">
            {entry.title}
          </p>
