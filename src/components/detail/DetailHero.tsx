...
export function DetailHero({ anime }: DetailHeroProps) {
  const [synopsisExpanded, setSynopsisExpanded] = useState(false);

  const synopsisText = anime.synopsis ?? "";
  const isLongSynopsis = synopsisText.length > 300;
  const displaySynopsis =
    isLongSynopsis && !synopsisExpanded
      ? synopsisText.slice(0, 300).trimEnd() + "..."
      : synopsisText;
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-8">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              {anime.score && (
                <span className="inline-flex items-center gap-1 rounded-lg bg-accent/10 px-3 py-1 text-accent font-semibold">
